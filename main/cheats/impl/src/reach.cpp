...
void reach::invoke(std::shared_ptr<c_context> ctx) {
	...
	jfieldID mouseover_fid = gasper::instance->get_env()->GetFieldID(mc_class, "s", "Lauh;");

	jobject new_raytrace = ctx->local->raytrace(m_distance);
	gasper::instance->get_env()->SetObjectField(mc, mouseover_fid, new_raytrace);
	...
	//gasper::instance->get_env()->DeleteLocalRef(new_raytrace);
