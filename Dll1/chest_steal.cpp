...
void chest_steal::invoke(std::shared_ptr<c_context> ctx) {
	...
	jclass ctnr_class = machip::instance->get_env()->FindClass("xo");
	...
	auto close_container = [ctx, ctnr_class, openctnr_obj]() {
		SendMessage(FindWindow("LWJGL", nullptr), WM_KEYDOWN, VK_ESCAPE, NULL);
		SendMessage(FindWindow("LWJGL", nullptr), WM_KEYUP, VK_ESCAPE, NULL);
	};
