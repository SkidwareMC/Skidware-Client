...
// Context
std::shared_ptr<wrapper::c_context> machip::hooks::gl_context = nullptr;
...
int __stdcall machip::hooks::swap_buffers_hk(HDC hdc)
{
	...
	static auto mc_window = wrapper::find_window(xorstr_("Minecraft 1.8.9"));
	auto window_rect = wrapper::get_window_rect(xorstr_("Minecraft 1.8.9"));
	...
	// make sure to dip
	if (!machip::instance || !machip::instance->is_running)
		return machip::hooks::oswap_buffers(hdc);
